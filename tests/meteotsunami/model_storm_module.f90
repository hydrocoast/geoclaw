...
module model_storm_module
    ...
    type model_storm_type
        ...
        ! Type 10: planewave pressure-wave parameters
        integer :: wave_shape = 1
        real(kind=8) :: wave_amplitude = 0.d0
        real(kind=8) :: wave_wavelength = 0.d0
        real(kind=8) :: wave_count = 1.d0
        real(kind=8) :: wave_speed = 0.d0
        real(kind=8) :: wave_origin(2) = [0.d0, 0.d0]
        real(kind=8) :: wave_direction(2) = [1.d0, 0.d0]
        real(kind=8) :: wave_cross_width = -1.d0
...
contains
    ...
    subroutine set_storm(storm_data_path, storm, storm_spec_type, log_unit)
        ...
        if (.not. module_setup) then
            if (storm_spec_type == 10) then
                call set_planewave_storm(storm_data_path, storm, log_unit)
                module_setup = .true.
                return
            end if
    ...
    ! ==========================================================================
    ! Read settings for a translating planewave pressure pulse
    ! ==========================================================================
    subroutine set_planewave_storm(storm_data_path, storm, log_unit)

        use utility_module, only: to_upper

        implicit none

        character(len=*), optional, intent(in) :: storm_data_path
        type(model_storm_type), intent(inout) :: storm
        integer, intent(in) :: log_unit

        integer, parameter :: data_file = 701
        integer :: io_status, line_status
        character(len=256) :: first_line, shape_name
        real(kind=8) :: theta_deg, theta_rad
        logical :: first_line_is_shape

        if (.not.present(storm_data_path)) then
            print *, "Type 10 storm requires a storm data file path."
            stop
        end if

        print *,'Reading planewave storm file ', storm_data_path
        open(unit=data_file, file=storm_data_path, status='old', action='read', &
             iostat=io_status)
        if (io_status /= 0) then
            print *, "Error opening storm data file. status = ", io_status
            stop
        ...
        shape_name = ""
        read(first_line, *, iostat=line_status) shape_name
        first_line_is_shape = .false.
        if (line_status == 0) then
            shape_name = to_upper(shape_name)
            select case(trim(shape_name))
                case("SINE", "SIN", "SINUSOIDAL")
                    storm%wave_shape = 1
        ...
        if (first_line_is_shape) then
            read(data_file, *, iostat=io_status) storm%wave_amplitude
            if (io_status /= 0) then
                print *, "Could not read wave_amplitude from storm file."
                stop
            end if
        else
            read(first_line, *, iostat=io_status) storm%wave_amplitude
            if (io_status /= 0) then
                print *, "Could not parse first line as wave_shape or wave_amplitude."
                stop
            end if
            storm%wave_shape = 1
        end if

        read(data_file, *, iostat=io_status) storm%wave_wavelength
        if (io_status /= 0) stop "Could not read wave_wavelength from storm file."
        read(data_file, *, iostat=io_status) storm%wave_count
        if (io_status /= 0) stop "Could not read wave_count from storm file."
        read(data_file, *, iostat=io_status) storm%wave_speed
        if (io_status /= 0) stop "Could not read wave_speed from storm file."
        read(data_file, *, iostat=io_status) storm%wave_origin
        if (io_status /= 0) stop "Could not read wave_origin from storm file."
        read(data_file, *, iostat=io_status) theta_deg
        if (io_status /= 0) stop "Could not read sine direction angle from storm file."
        read(data_file, *, iostat=io_status) storm%wave_cross_width
        if (io_status /= 0) stop "Could not read wave_cross_width from storm file."
        close(data_file)

        if (storm%wave_wavelength <= 0.d0) then
            print *, "wave_wavelength must be > 0 for storm_specification_type == 10."
            stop
        end if
        if (storm%wave_count <= 0.d0) then
            print *, "wave_count must be > 0 for storm_specification_type == 10."
            stop
        end if
        if (storm%wave_speed == 0.d0) then
            print *, "Warning: wave_speed is zero; pressure pulse is stationary."
        end if

        theta_rad = theta_deg * atan(1.d0) / 45.d0
        storm%wave_direction = [cos(theta_rad), sin(theta_rad)]

        ! write(log_unit, *) "Type 10 planewave pressure wave"
        ! write(log_unit, "('  amplitude [Pa] = ', d16.8)") storm%wave_amplitude
        ! write(log_unit, "('  wavelength [m] = ', d16.8)") storm%wave_wavelength
        ! write(log_unit, "('  speed [m/s] = ', d16.8)") storm%wave_speed
        ! write(log_unit, "('  origin [m] = ', 2d16.8)") storm%wave_origin
    ...
    end subroutine set_planewave_storm
    ...
    ! ==========================================================================
    ! Type 10: translating planewave pressure pulse with no wind
    ! ==========================================================================
    subroutine set_planewave_fields(maux, mbc, mx, my, xlower, ylower,    &
                                        dx, dy, t, aux, wind_index,            &
                                        pressure_index, storm)

        use geoclaw_module, only: Pa => ambient_pressure
        use geoclaw_module, only: coordinate_system, spherical_distance
        use geoclaw_module, only: latlon2xy, xy2latlon
        ...
        do j = 1-mbc, my+mbc
            ...
            do i = 1-mbc, mx+mbc
                x = xlower + (i - 0.5d0) * dx
                if (coordinate_system == 2) then
                    ! Project lon/lat displacement to local east-north meters.
                    dx_local = sign(spherical_distance(x, travel(2), travel(1), travel(2)), &
                                    x - travel(1))
                    dy_local = sign(spherical_distance(travel(1), y, travel(1), travel(2)), &
                                    y - travel(2))
                    xi = dx_local * tangent(1) + dy_local * tangent(2)
                    eta = dx_local * normal(1) + dy_local * normal(2)
                else
                    xi = (x - travel(1)) * tangent(1) + (y - travel(2)) * tangent(2)
                    eta = (x - travel(1)) * normal(1) + (y - travel(2)) * normal(2)
                end if
                ...
                if (-total_length <= xi .and. xi <= 0.d0) then
                    if (storm%wave_cross_width <= 0.d0 .or.                   &
                        ...
                        phase = -xi / storm%wave_wavelength
                        select case(storm%wave_shape)
                            case(2) ! repeating triangular wave (peak + trough per wavelength)
                                local_phase = phase - floor(phase)
                                if (local_phase < 0.25d0) then
                                    pulse = 4.d0 * local_phase
                                else if (local_phase < 0.75d0) then
                                    pulse = 2.d0 - 4.d0 * local_phase
                                else
                ...
                aux(pressure_index, i, j) = Pa + storm%wave_amplitude * pulse
            end do
        end do

    end subroutine set_planewave_fields
