...
contains
    ...
    subroutine set_storm(data_file)
        ...
        use model_storm_module, only: set_planewave_fields
        ...
        if (.not.module_setup) then
            ...
            if (storm_specification_type == 10) then
                set_model_fields => set_planewave_fields
                call set_model_storm(storm_file_path, model_storm,         &
                                     storm_specification_type, log_unit)
            else if (0 < storm_specification_type .and. storm_specification_type <= 9) then
